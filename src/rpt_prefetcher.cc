...
//#include "rpt_RPTEntry.h"
//#include "rpt_RPTMap.h"

#define ENTRY_LIMIT 100
using namespace std;

struct RPTentry{
   RPTentry(Addr pc) : pc(pc), lastAddress(0), delta(0), next(0), prev(0){ } // Constructor
...
struct RPTmap{
   RPTmap() : amountofentries(0), head_ptr(0), tail_ptr(0){ } // Constructor
   RPTentry *find_entry(Addr pc); // Pointer object from the other struct
...
/* This is the pinitialilace to ze data structures. */
